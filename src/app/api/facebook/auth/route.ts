...
export async function GET(req: NextRequest) {
  ...
  // Allow caller to specify where to redirect after OAuth completes
  const returnTo = req.nextUrl.searchParams.get("returnTo") || "/ads"

  const baseUrl = process.env.NEXTAUTH_URL || process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"
  const redirectUri = `${baseUrl}/api/facebook/auth/callback`

  const state = crypto.randomBytes(32).toString("hex")
  // Encode org + user info + return destination in state for the callback
